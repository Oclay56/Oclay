...
def test_candidate_score_ignores_legacy_historical_signal():
    base_candidate = {
        "odds": 1.9,
        "side": "under",
        "line": 0.5,
    ...
    assert "historicalScoreAdjustment" not in with_history
    assert with_history["score"] == without_history["score"]
    assert "historicalCalibration" not in with_history["probabilityAssessment"]
    assert "historical_positive_signal" not in with_history["reasonTags"]
    assert "historical_sample_gated_score_adjustment" not in with_history["reasonTags"]


def test_compact_response_omits_legacy_historical_enrichment_status():
    ...
    assert "historicalEnrichmentStatus" not in row
    assert "historicalEnrichmentCoverage" not in row
    assert "historicalEnrichedLegs" not in row
