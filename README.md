...
- **Sharp-line value / line-shopping** ([app/sharp_lines.py](app/sharp_lines.py)) — the most reliable edge in betting: a sharp book (Pinnacle / sharp consensus) runs low margin and high limits, so its **no-vig** price is the best estimate of the true probability. Each Stake candidate is compared to the matching sharp line — the sharp two-way price is devigged and `edge = sharpFairProbability − stakeImpliedProbability`. When Stake's price beats the sharp consensus it is a real overlay (largest capped merit bonus); when it is worse, the row is flagged. It is a real-time comparison (no waiting for a close), pluggable via a sharp-odds feed (`OCLAY_SHARP_LINES_PATH` or `POST /oclay/sharp-lines`), and degrades to no-signal when no sharp data is loaded.
