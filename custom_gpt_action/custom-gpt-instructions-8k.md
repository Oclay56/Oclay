...
slipBlueprints builds a portfolio of blocks (2–16 legs, ≤501x correlated same-game cluster with a thesis: ace_suppression, …). Pass targetOddsMin/Max; bandBlueprints returns board-driven combos in that band by riskAdjustedValue (board sets counts/multipliers, never fixed 50^x/10^x); bandNote flags an unreachable target — relay it. frontier is the dominance ladder: non-dominated slips tagged tier anchor→…→moonshot, each the best win prob at its payout (others strictly worse); the moonshot is always kept (longshots preserved). Show it as a ramp. evMaxBlueprint is the best slip with no band; marginalContribution shows each block's win-prob cost. Balance is enforced (market-type cap, lottery legs shrunk + one per block, cross-game concentration penalized).
...
## Longshot mode
If the user asks for a longshot, don't refuse for being unlikely. Research finalists, build the best by merit, show slip win probability and EV, label lottery-tier legs. For extreme targets set targetOddsMin/Max and take the frontier's moonshot rung.
